...
class Land extends ListUpdater {
  async onFetchSuccess(text, element, url) {
    ...
    const cadastralNumber =
      this.getValueByLabel(text, "Кадастровый номер", false) || null;
    let storageItem = {
      size,
      description,
    };
    if (cadastralNumber || this.extractCadastralNumber(description)) {
      const processedCadastralNumber =
        this.extractCadastralNumber(cadastralNumber) ||
  ...
  async onReadLocalStorage(element, url) {
    ...
    if (cadastralNumber && !coordsWGS84) {
      ...
      localStorage.setItem(
        url,
        JSON.stringify({ ...storageItem, coordsWGS84 })
      );
    }
    this.insertChanges(element, storageItem);
  ...
  async getWGS84Coords(cadastralNumber) {
    ...
    try {
      let response = await fetch(
        "https://brok.land/api/v1/proxy/elandpov-info-by-cadastre/",
        {
          method: "POST",
          mode: "cors",
          headers: {
  ...
  async insertGoogleMapLinkNode(element, coordsWGS84) {
    ...
    const locationName = $(element)
      .find('.breadcrumb.x-normal i[data-icon="location-filled"]')
      .eq(0)
      .parent()
      .text();
    ...
    const lat =
      (Math.atan(Math.exp((coordsWGS84.y * Math.PI) / 20037508.34)) * 360) /
        Math.PI -
      90;
    const link = `<br />
  ...
  extractCadastralNumber(text) {
    ...
    if (cadastralRegexp.test(text)) {
      ...
      if (cadastralRegexpNoDots.test(text)) {
        cadastralNumber =
          text.match(cadastralRegexpNoDots)[0].substr(0, 10) +
          ":" +
          text.substr(10, 2) +
          ":" +
          text.substr(12, 3) +
