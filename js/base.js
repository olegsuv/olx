  ...
  checkLoads() {
    ...
    if (this.offers.length === this.modified) {
      this.log(`Loading finished: 
                ...
                fetch loads ${this.fetchLoads} (${parseInt(
        (this.fetchLoads / this.offers.length) * 100,
        10
      )}%),
                localStorage loads ${this.localStorageLoads} (${parseInt(
        (this.localStorageLoads / this.offers.length) * 100,
        10
      )}%)`);
      this.reset();
    }
